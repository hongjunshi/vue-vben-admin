<template>
  <PageWrapper dense contentFullHeight fixedHeight contentClass="flex">
    ...
    <BasicTable @register="registerTable" class="w-3/4 xl:w-4/5" :searchInfo="searchInfo">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate" v-auth="'system:users:create'"
          >新增用户
        </a-button>
      </template>
...
<script lang="ts">
  ...
  import DepartmentModal from './UserModal.vue';

  import { columns, searchFormSchema } from './user.data';
  import { useGo } from '/@/hooks/web/usePage';
  import { RouteLocationRaw } from 'vue-router';
  import OrganizationTree from '../components/OrganizationTree.vue';
  import { deleteUserById, listUsers } from '/@/api/system';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      let tableProps = getTableSetting({
        searchPermissionCode: 'system:users:search',
        title: '用户列表',
        api: listUsers,
        rowKey: 'id',
        columns,
      ...
      function handleSelect(selectedNode: OrganizationSelectedNode) {
        ...
        searchInfo.search_departmentIds = undefined;
        searchInfo.search_departmentId = undefined;
        if (selectedNode.type === OrganizationNodeType.ORGANIZATION) {
          ...
          selectedNode.keys.length > 1
            ? (searchInfo.search_departmentIds = selectedNode.keys)
            : (searchInfo.search_departmentId = selectedNode.keys[0]);
          currentOrganizationNode.organizationId = selectedNode.node.organizationId;
